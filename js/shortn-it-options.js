jQuery( document ).ready( function( $ ) {
	...
	$( '#shortn_it_permalink_custom' ).keyup( function( e ) {
		var $this = $( this ),
			val = $this.val().replace( /^\/+/g, '' ).replace( /[^a-zA-Z0-9-_\/]/g, '' );
		$this.val( val );
		$( '.shortn-look-like .prefix' ).text( val );
	} );
	...
	$( 'input[name=shortn_it_permalink_prefix]' ).change( function( e ) {
		if( $( 'input[name=shortn_it_permalink_prefix]:checked' ).val() == 'custom' ) {
			...
			$( '.shortn-look-like .prefix' ).text( $( '#shortn_it_permalink_custom' ).val().replace( /^\/+/g, '' ) ).replace( /[^a-zA-Z0-9-_\/]/g, '' );
		}
